...
def APICall(url, absolute=False, method=requests.get):
    ...
    while retry:
        ...
        try:
            ...
            errmsg = "Connection Exception: Request exception caught and will be retried: `{}` Retry: {}/{}".format(e.message, retries, RETRIES_ALLOWED)
        ...
        if code >= 200 and code < 300:
            ...
            errmsg = "401 Authorization error: Problem with API Call. Getting new token Retry#: {1}/{2}".format(RETRY_DELAY, retries, RETRIES_ALLOWED)
            tokenator.reset()
            tokenator.getToken()
...
def latestMetricInstances(insts, single=False):
    ...
    for idx, inst in enumerate(insts):
        gendate = filter(lambda x: x['name']==DATECREATEDFIELD, inst['values'])
        try:
