...
from lib.metricxmloutput import DATECREATEDFIELD, VERSIONFIELD
...
def writeMetricsToXML(dataObj, dataStats, xmlFilePath):
    ...
    if "layers" in dataStats:
        for layername, value in dataStats['layers'].iteritems():
    ...
    for name,layer in dataObj.iteritems():
...
def writeDictionaryToXML(parentNode, dValues):
    ...
    for xmlTag, item in dValues.iteritems():
