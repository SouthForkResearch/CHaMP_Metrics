...
class CHaMPSurvey(object):
    ...
    def load_topo_project(self, directory, visitID):
        ...
        self.DEM = CHaMP_DEM("DEM", getPath("DEM"))
        self.DetrendedDEM = CHaMP_DetrendedDEM("DetrendedDEM", getPath("DetrendedDEM"))
        self.WaterDepth = CHaMP_WaterDepth("WaterDepth", getPath("WaterDepth"))
        self.ErrorSurface = CHaMP_ErrorSurface("ErrorSurface", getPath("ErrorSurface"))
        self.WaterSurfaceDEM = CHaMP_WaterSurfaceDEM("WaterSurfaceDEM", getPath("WaterSurfaceDEM"))
        self.AssocPointQuality = CHaMP_Associated_3DPointQuality("AssocPointQuality", getPath("AssocPointQuality"))
        self.AssocSlope = CHaMP_Associated_Slope("AssocSlope", getPath("AssocSlope"))
        self.AssocRough = CHaMP_Associated_Roughness("AssocRough", getPath("AssocRough"))
        self.AssocPointDensity = CHaMP_Associated_PointDensity("AssocPointDensity", getPath("AssocPointDensity"))
        self.AssocInterpolationError = CHaMP_Associated_InterpolationError("AssocInterpolationError", getPath("AssocInterpolationError"))

        self.Topo_Points = CHaMP_TopoPoints("Topo_Points", getPath("Topo_Points"))
        self.Topo_Points.dem = self.DEM.filename

        self.StreamFeatures = CHaMP_StreamFeature_Points("StreamFeatures", getPath("StreamFeatures"))
        self.EdgeofWater_Points = CHaMP_EdgeofWater_Points("EdgeofWater_Points", getPath("EdgeofWater_Points"))
        self.EdgeofWater_Points.dem = self.DEM.filename

        self.Control_Points = CHaMP_ControlPoints("Control_Points", getPath("Control_Points"))
        self.Error_Points = CHaMP_Error_Points("Error_Points", getPath("Error_Points"))

        self.Breaklines = CHaMP_Breaklines("Breaklines", getPath("Breaklines"))
        self.Breaklines.survey_points = self.Topo_Points.get_list_geoms() + self.EdgeofWater_Points.get_list_geoms()

        self.WaterExtent = CHaMP_WaterExtent("WaterExtent", getPath("WaterExtent"), "Wetted")
        ...
        self.BankfullExtent = CHaMP_WaterExtent("BankfullExtent", getPath("BankfullExtent"), "Bankfull")
        ...
        self.WettedIslands = CHaMP_Islands("WettedIslands", getPath("WettedIslands"), "Wetted")
        self.BankfullIslands = CHaMP_Islands("BankfullIslands", getPath("BankfullIslands"), "Bankfull")
